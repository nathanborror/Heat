...
final class ConversationViewModel {
    ...
    func generate(chat prompt: String, context: [String: String] = [:], toolChoice: Tool? = nil, agentID: String? = nil) throws {
        guard let conversationID else { return }
        generateTask = try API.shared.generate(conversationID: conversationID, prompt: prompt, context: context, toolChoice: toolChoice, agentID: agentID)
    }
    ...
    func generate(chat prompt: String, images: [Data], context: [String: String] = [:]) throws {
        guard let conversationID else { return }
        generateTask = try API.shared.generate(conversationID: conversationID, prompt: prompt, images: images, context: context)
    }
